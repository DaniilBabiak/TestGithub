...
export class ChallengesComponent implements OnInit {
  ...
  getChallenges(): void {
    this.challengeService.getChallenges(this.config.currentPage, this.config.itemsPerPage, this.sortConfig)
      .subscribe((response: PaginatedResult<Challenge[]>) => {
        this.challenges = response.resut;
        this.config = {
          itemsPerPage: 5,
  ...
  disableChallenge(challenge: Challenge): void {
    this.challenges = this.challenges.filter(c => c !== challenge);
    challenge.status = 'Disabled';
    this.challengeService.updateChallenge(challenge).subscribe(() => window.location.reload());
  }


  enableChallenge(challenge: Challenge): void {
    this.challenges = this.challenges.filter(c => c !== challenge);
